    ...
    try {
      if (address) {
        ...
        if (_messages.length === 1) {
          const { chatHistories, createdChatHistory } = (
  ...
  return (
    <Stack flexGrow={1} sx={{ height: "100%", bgcolor: grey[900] }}>
      ...
      <Stack
        ...
        px={3}
        pt={3}
        sx={{ overflowY: "auto", height: "100px" }}
        ref={chatBoxRef}
      >
        {messages.map((message, index) => (
          <Stack direction="row" spacing={1} key={index}>
            {message.role !== "user" ? (
              <Avatar src="/assets/images/gpt.png" alt="GPT" />
            ) : (
              <Avatar src="/assets/images/user.png" alt="User" />
            )}
